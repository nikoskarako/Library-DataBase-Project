...
if (isset($_POST['enableButton'])) {
  ...
  $updateQuery = "UPDATE reservation SET activation = 'enabled' WHERE user_id = '$borrowingId' AND isbn = '$isbn'";
  $updateResult = mysqli_query($connection, $updateQuery);

  // Check if the update query was successful
  if ($updateResult) {
    $success = "Reservation enabled successfully.";

    // Refresh the page after updating
    echo '<script type="text/javascript">
            window.location.href = "controller_reservation.php";
          </script>';
  } else {
        ...
        <tbody>
          <?php foreach ($borrowings as $borrowing): ?>
            <tr>
              ...
              <td><?php echo $borrowing['date_of_reservation']; ?></td>
              <td><?php echo $borrowing['isbn']; ?></td>
              <td><?php echo $borrowing['last_update']; ?></td>
              <td><?php echo $borrowing['expire_date']; ?></td>
